...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	// active 
 	if($row[4] == 1) { 		
 		$paymentStatus = "<label class='label label-success'>Successfully Ordered</label>";
 	} else if($row[4] == 2) { 		
 		$paymentStatus = "<label class='label label-info'>On Transit</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-warning'>Ordered Received</label>";
 	} // /else
